...
class LedgerController implements ExternalSignerController {
  ...
  constructor() {
    ...
    this.isSpeculos = IS_SPECULOS_TRANSPORT
  ...
  static isConnected = async () => {
    if (IS_SPECULOS_TRANSPORT) return true
   ...
   *
   * Was removed in v5.24.0, because after an update of the Ledger DMK - the
   * observable returned by the Ledger SDK started to throw WITHOUT an awkward
   * delay there was. But then brought back in v5.33.5, because after another
   * update of the Ledger DMK - the DMK behavior turned out to be inconsistent
   * (not working while in the middle of signing a raw transaction).
   */
  ...
  async #initSDKSessionIfNeeded() {
    ...
    try {
      ...
      if (this.isSpeculos) {
        dmkBuilder.addTransport(speculosTransportFactory(SPECULOS_HTTP_URL))
      } else {
