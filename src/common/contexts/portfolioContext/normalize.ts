...
export function normalizeResponse(response: ResponseV2): ResponseV1 {
  ...
  if (response.data.nfts) {
    const nftAssets: NFTV1[] = response.data.nfts.map((nft) => {
      return {
        type: 'nft',
        network: response.data.network,
        address: nft.address,
        ...
        tokenImageUrl: nft.assets[0]?.token_url || '', // Assuming there's at least one asset.
        category: 'NFT',
        shouldDisplay: nft.shouldDisplay,
        collectionId: nft.collectionId,
        collectionName: nft.collectionName,
        collectionHidden: nft.collectionHidden,
        collectionImg: nft.assets[0]?.token_url || '', // Assuming there's at least one asset.
        collection: {
