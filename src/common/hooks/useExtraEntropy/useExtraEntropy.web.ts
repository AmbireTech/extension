...
// samples fill the pool sooner than twenty expensive ones, and cost ~1ms of CPU to fold. pressure is
// a flat constant for a mouse, but a sensor reading for touch and pen, and so noise that does not
// follow from the path the way the coordinates do.
...
// Its budget is its own, so the pointermove burst that fills within seconds of the page opening
 ...
 * exists. Here it is fed from pointer movement - mouse, touch or pen - and keystroke timing, both
 * observed on `document`.
 *
...
const useExtraEntropy = () => {
  ...
  // last sample in React state, and that update ran on every mouse move and rippled through the
