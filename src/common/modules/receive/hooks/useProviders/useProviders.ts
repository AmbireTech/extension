...
const useProviders = ({ walletAddress, networkId }: UseProvidersProps): UseProvidersReturnType => {
  ...
  const openRampNetwork = (name: string) => {
    ...
    navigate(ROUTES.provider, {
      state: {
        name,
        uri: `https://app.ramp.network/?userAddress=${walletAddress}&hostApiKey=${RAMP_HOST_API_KEY}&swapAsset=${assetsList[networkId]}&finalUrl=ambire://&hostAppName=Ambire&hostLogoUrl=https://www.ambire.com/ambire-logo.png`
      }
    })
  }
