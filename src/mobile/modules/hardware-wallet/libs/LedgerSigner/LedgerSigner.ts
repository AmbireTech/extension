...
import LedgerController, {
  LedgerSignature
} from '@mobile/modules/hardware-wallet/controllers/LedgerController'

// Mobile counterpart of the web LedgerSigner. Identical signing logic; the only
// difference is that it talks to the mobile (bridged) LedgerController and, since
// that controller has no DMK `walletSDK`, the post-unlock sanity check is the
// `isUnlocked` path/address verification instead of a `walletSDK` truthiness check.
class LedgerSigner implements KeystoreSignerInterface {
  ...
  // @ts-expect-error param is the concrete LedgerController, not the interface
