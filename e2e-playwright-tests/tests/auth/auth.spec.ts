...
test.describe('trezor', () => {
  ...
  test('should successfully authenticate using Trezor and import existing accounts', async ({
    ...
    await test.step('make sure accounts are imported', async () => {
      ...
      await expect(page.getByText(mainConstants.addresses.trezorAccount1)).toContainText('0x3f2329C9')
      await expect(page.getByText(mainConstants.addresses.trezorAccount2)).toContainText('0x4f4F1488')
    })
  })
})
